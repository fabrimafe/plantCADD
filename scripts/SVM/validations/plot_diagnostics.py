...
def plot_prediction_quantiles_by_feature_barstacked(df,disc_col, sparse=False,prefix=''):
    title=f'NN prediction score quantiles colored by {disc_col}'
    figname=f'{path}/feat_plots/bstack_by_{disc_col}.svg'
    if sparse:
    ...
    df['Prediction_quantiles'] = pd.qcut(df['Prediction'], q=25, labels=False)
    df['Prediction_quantiles'] = df['Prediction_quantiles'] + 1
    
    # Group the data by quantiles and Genomic_Region
    grouped = df.groupby(['Prediction_quantiles', disc_col]).size().reset_index(name='count')
    grouped['Proportion']=grouped['count']/grouped['count'].sum()
    grouped['Proportion'] = grouped['Proportion'] / grouped.groupby('Prediction_quantiles')['Proportion'].transform('sum')
    pivot = grouped.pivot(index='Prediction_quantiles', columns=disc_col, values='Proportion')
    color_dict=['C0','C1','C2','C3','C4','black','C5','C6','C7','C8','C9','lime']
    
    ax = pivot.plot(kind='bar', stacked=True, color=color_dict)
    ax.set_xlabel('Prediction Score Quantiles')
    ax.set_ylabel('Proportion')
    ax.legend(loc='center left', bbox_to_anchor=(1.05, 0.5))
    
    custom_tick_positions = [4, 9, 14, 19, 24, 29]
    custom_tick_labels = ['20%', '40%', '60%', '80%', '100%']
...
# Adding regulatory regions to the plot
print('test')
...
#plot_prediction_quantiles_by_feature_barstacked(df,'parsed_genomic_region',False)
plot_prediction_quantiles_by_feature_barstacked(df_novel,'parsed_genomic_region',False,'NOVEL')
#plot_prediction_quantiles_by_feature_barstacked(df_neutral,'parsed_genomic_region',False,'NEUTRAL')
plot_prediction_quantiles_by_feature_barstacked(df_novel,'label',False)

print('Plotted genomic_regions.')

df_CDS=df[df.genomic_region=='CDS']
df_CDS_novel=df_CDS[df_CDS.label==1]
#df_CDS_neutral=df_CDS[df_CDS.label==0]
#plot_prediction_quantiles_by_feature_barstacked(df_CDS,'VARIANT_TYPE',False)
